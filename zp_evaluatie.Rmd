...
# create a number of variables to include in the later process rapport
...
## Step 3. define the total number of unique GPs who prescribed a test and total degree of participation
unique.gp.bnp <- nrow(distinct(as.tibble(df.bnp$naam)))
participation.total.bnp <- round(unique.gp.bnp/total.gp, 3)*100

## Step 4. define the median number of tests per prescribing GP
df.bnp4 <- df.bnp%>%
  group_by(naam)%>%
  summarize(aantal = n())
